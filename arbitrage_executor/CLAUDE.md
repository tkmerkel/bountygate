...
| `chrome_helpers.py` | Chrome/CDP launch. Do not modify launch behavior |
...
| `⚠️ WARNING` | A specific opportunity was skipped. No money at risk. Common causes: selectors missing for a market, ROI dropped below threshold mid-execution, BetMGM rejected the bet. | Read the message. If selectors are missing, follow `SOP.md § 2 — Map the broken selector with Claude Code`, then `python validate_selector.py --site <site> --market <market>`. If it's ROI/limit related, no action needed — the next opportunity will be tried. |
...
4. Investigate why the hedge failed (check `logs/execution_failures.log` and the audit dir referenced in the Discord message). Common causes: FanDuel UI changed (see `SOP.md § 2`), Chrome lost connection, FanDuel suspended the market.
