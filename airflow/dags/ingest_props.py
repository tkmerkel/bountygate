"""Read-only The Odds API props ingestion poller. Every 15 min -> raw_market_snapshots."""
...
@dag(
    ...
    tags=["ingest", "the_odds_api", "props"],
)
def ingest_props():
    @task(outlets=[RAW_ASSET])
    def fetch_and_land() -> int:
        records = get_connector("the_odds_api_props").fetch_snapshots()
        n = land_raw(records)
        print(f"[ingest_props] landed {n} records")
        return n
